...
public class PlayerController : MonoBehaviour
{
    ...
    public GameObject[] enemiesToDefeat; // <<< ADD THIS: Assign in Inspector
    private bool bossSceneLoaded = false; // <<< Prevent multiple loads
    ...
    void Update()
    {
        ...
        CheckEnemiesDefeated(); // <<< Check every frame
    }

    private void CheckEnemiesDefeated()
    {
        if (bossSceneLoaded || enemiesToDefeat == null || enemiesToDefeat.Length == 0)
            return;
        ...
        if (allDefeated)
        {
            bossSceneLoaded = true;
            Invoke("LoadBossScene", 2f); // <<< Delay for animation or effect
        }
    }
